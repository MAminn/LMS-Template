  ...
  try {
    ...
    const course1 = await prisma.course.create({
      data: {
        ...
        description: 'Master HTML, CSS, JavaScript, React, Node.js, and MongoDB in this comprehensive full-stack development course. Build real-world projects and deploy them to production.',
    ...
    const course2 = await prisma.course.create({
      data: {
        ...
        description: 'Learn SEO, Social Media Marketing, Content Strategy, Email Marketing, and Analytics. Grow your business with proven digital marketing strategies.',
    ...
    const course3 = await prisma.course.create({
      data: {
        ...
        description: 'Master data analysis, machine learning, and data visualization using Python, Pandas, NumPy, and Scikit-learn. Work with real datasets.',
    ...
    await prisma.lesson.createMany({
      data: [
        ...
        },
        {
          ...
          content: 'Flexbox is a powerful layout method in CSS that allows you to design flexible responsive layout structure...',
    ...
    if (lessons.length > 0) {
      await prisma.lessonProgress.createMany({
        data: [
          // Student 1 progress
          { studentId: student1.id, lessonId: lessons[0].id, completed: true, completedAt: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) },
          { studentId: student1.id, lessonId: lessons[1].id, completed: true, completedAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000) },
          { studentId: student1.id, lessonId: lessons[2].id, completed: true, completedAt: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000) },
          
          // Student 2 progress
          { studentId: student2.id, lessonId: lessons[0].id, completed: true, completedAt: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000) },
          { studentId: student2.id, lessonId: lessons[1].id, completed: true, completedAt: new Date(Date.now() - 8 * 24 * 60 * 60 * 1000) },
          { studentId: student2.id, lessonId: lessons[2].id, completed: true, completedAt: new Date(Date.now() - 6 * 24 * 60 * 60 * 1000) },
          { studentId: student2.id, lessonId: lessons[3].id, completed: true, completedAt: new Date(Date.now() - 4 * 24 * 60 * 60 * 1000) }
        ]
      });
    }
            ...
            create: [
              {
                ...
                options: {
                  create: [
                    ...
                    { text: 'Hyperlink and Text Markup Language', isCorrect: false, order: 4 }
                  ]
                }
              },
              {
      ...
      await prisma.quizAttempt.create({
        data: {
          ...
          answers: {
            create: [
              {
                questionId: (await prisma.quizQuestion.findFirst({ where: { quizId: quiz1.id, order: 1 } }))!.id,
                selectedText: 'HyperText Markup Language',
                isCorrect: true,
                points: 1
              },
              {
                questionId: (await prisma.quizQuestion.findFirst({ where: { quizId: quiz1.id, order: 2 } }))!.id,
                selectedText: '<h1>',
                isCorrect: true,
                points: 1
              },
              {
                questionId: (await prisma.quizQuestion.findFirst({ where: { quizId: quiz1.id, order: 3 } }))!.id,
                selectedText: 'True',
                isCorrect: false,
                points: 0
              }
    ...
    await prisma.landingPageContent.create({
      data: {
        ...
        heroSubtitle: 'Experience the future of education with our cutting-edge Learning Management System. Interactive courses, real-time progress tracking, and professional certification.',
        ...
        featuresSubtitle: 'From interactive video lessons to comprehensive assessments, we provide all the tools for successful learning outcomes.',
        demoTitle: 'Experience The Academy',
        demoSubtitle: 'Try our platform with these demo accounts and see the power of modern learning technology.',
        footerDescription: 'Empowering learners worldwide with cutting-edge educational technology. Build, customize, and scale your learning platform with ease.',
        isActive: true,
        createdBy: admin.id
      }
    });
